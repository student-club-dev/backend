...
/** Device-session management for the provider app (D3). `AuthService` is bound to the `business_owners` tables. */
@ApiTags('Auth — Business Sessions')
@ApiBearerAuth()
@ApiUnauthorizedEnvelope()
@UseGuards(JwtAuthGuard)
@Controller('auth/business/sessions')
export class BusinessSessionsController {
  ...
  @ApiNotFoundEnvelope(
    ...
    'No session with this id for the caller.',
    'Sessiya topilmadi',
  )
