...
import { CreateListingInput } from '../../application/listings.io';
...
/**
 * CreateListingRequestDto — the create payload (matches elon-uz.json). Required: `categoryKey`,
 * `title`, `images`, `priceUnit`, `originalPrice`, `discount`, `redemption`, `validFrom`, `validTo`.
 * `finalPrice`, `status`, `usedCount`, `viewsCount` are server-owned and never accepted here.
 * Business rules (title length, positive price, validity window, pricing, attributes, option-group
 * coherence, branch ownership) are enforced in the service (LISTINGS.md §10).
 */
